...
- **📖开箱即用**：内置基础工具、基础模型和基础LLM接口，无需配置，开箱即用；
- **🔗离线应用**：支持本机离线处理，基于本地Ollama大模型进行翻译；
...
- **🗽免费模型**：默认支持免费注册硅基流动的LLM账号，使用硅基流动上的免费大模型；
- **🎶批量处理**：支持批量视频处理，无需人工值守，系统会自动完成所有视频的字幕生成；
- **🦑多模式翻译**：支持直译、意译、深度思考翻译，适应不同场景下的需求；
- **👑专业字幕**：通过LLM大模型处理，让字幕更自然流畅，更地道专业；
- **🪧多语言支持**：源视频语言可以支持上百种（自动模式），翻译的目标语言支持10多种。
